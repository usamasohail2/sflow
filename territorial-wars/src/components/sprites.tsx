...
/** Photo cutout sprite (Wikimedia Commons — see public/sprites/ATTRIBUTION.md). */
function CarPhotoSprite({
  src,
  className = "",
  title,
  width = 56,
  height = 34,
}: SpriteProps & { src: string; width?: number; height?: number }) {
  return (
    // eslint-disable-next-line @next/next/no-img-element -- small local PNG sprites
    <img
      src={src}
      alt={title ?? "Car"}
      title={title}
...
/** Compact sedan — Honda Civic flex (real photo cutout). */
...
/** Mid-size SUV — Toyota Prado (real photo cutout). */
...
/** Flagship SUV — Land Cruiser flex (real photo cutout). */
export function LandCruiserSprite({
  ...
  return (
    <CarPhotoSprite
      src="/sprites/cars/landcruiser.png"
      className={className}
      title={title}
      width={60}
      height={34}
